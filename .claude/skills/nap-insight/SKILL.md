...
- 🗂️ **Đồng bộ LLM Wiki Pattern (provenance = `Sources:`):** mỗi insight nạp phải **truy ngược được nguồn gốc** (tên file + id + ngày sửa — đúng tinh thần `Sources:` trích ngược RAW) và **giữ NGUYÊN VĂN ý gốc, không bóp méo**. Đây là cùng luật "không bịa + truy nguồn" của hệ tri thức (xem CLAUDE.md mục 🗂️).
