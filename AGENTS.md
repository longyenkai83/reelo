...
# AGENTS.md — Reelo (engine dùng chung)
...
- **`.Codex/`** = ENGINE (skills · agents). **`engine/cong-thuc-viral/`** = công thức chung (hook · tiêu đề · tâm lý · critic) + **`mau-brand/`** (template tạo brand mới).
...
## 2. NGHI THỨC PHIÊN (Codex tự làm, không bắt chị nhắc)
**Đầu phiên:**
...
- [ ] 🔴 **Vệ sinh PHIÊN (không chỉ file .md):** nếu Codex tự *resume* một phiên đã chạy NHIỀU NGÀY → **đừng dùng lại, mở phiên MỚI hoặc `/clear`**. File lịch sử phiên ở `~/.Codex/projects/` (NGOÀI Reelo — KHÔNG đụng); chỉ cần bắt đầu phiên mới là né. *(2026-07-22: một phiên **71,5MB / 14.442 tin / 26 ngày** không clear → "Prompt too long" NGAY câu đầu, tràn trước khi làm gì. Nguyên nhân gốc là PHIÊN phình, không phải file .md.)*
...
`README.md` (dùng hằng ngày + cài máy mới) · `HUONG-DAN-KE-HOACH-NOI-DUNG.md` (kế hoạch 30 ngày) · `SOP-MASTER.md` (bản đồ kiến trúc + 7 chặng) · `.Codex/skills/viet-script/SKILL.md` (quy trình viết).
