...
from .creative_plan import validate_proposal
from .stages import stage_identity, validate_envelope, persist
from .context_packs import registry, validate_recipe
from .purified_plan import PurifiedPlan, ROUTE


def execute_plan(execution, context, work, assets, invoke, store):
    schema = PurifiedPlan.model_json_schema()
    inputs = dict(plan_schema=schema, asset_roles=registry(assets), plan_route=ROUTE)
    ...
    try:
        ...
        if raw.get('schema_version') != ROUTE: raise ValueError('purified_plan_route_required')
